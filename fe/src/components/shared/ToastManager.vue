    ...
    <Teleport to="body">
        <TransitionGroup name="toast" tag="div" class="fixed top-4 right-4 z-[9999] space-y-3">
            ...
            <div v-for="toast in toasts" :key="toast.id"
                ...
                <!-- Icon -->
                <div class="flex-shrink-0" :aria-label="getToastTypeLabel(toast.type)">
                    <!-- Success Icon -->
                    ...
                    <!-- Warning Icon -->
                    <svg v-else class="w-6 h-6 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button @click="removeToast(toast.id)"
                    ...
                    :aria-label="$t('Shared.ToastManager.close')">
...
/**
 * Get label for toast type (for accessibility)
 */
const getToastTypeLabel = (type) => {
    const labels = {
        success: t('Shared.ToastManager.success'),
        error: t('Shared.ToastManager.error'),
        info: t('Shared.ToastManager.info'),
        warning: t('Shared.ToastManager.warning')
    }
