  ...
  <AdminLayout>
    ...
    <div class="mb-6 flex items-center justify-between">
      <div>
        <div class="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 class="text-2xl font-semibold text-gray-900">{{$t('Admin.Categories.title')}}</h1>
            <p class="text-gray-600 mt-1">{{$t('Admin.Categories.subtitle')}}</p>
          </div>
        </div>
      </div>
      <div class="flex items-center gap-3">
        <div class="relative w-80">
          <input v-model="keyword" type="text" :placeholder="$t('Admin.Categories.searchPlaceholder')"
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-gray-600 mb-1">{{$t('Admin.Categories.total')}}</p>
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-gray-600 mb-1">{{$t('Admin.Categories.active')}}</p>
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-gray-600 mb-1">{{$t('Admin.Categories.inactive')}}</p>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div class="flex items-center justify-between">
              ...
              <div class="flex-1">
                <div class="flex items-center gap-3 mb-2">
                  ...
                  <span
                    ...
                    {{ category.is_active ? $t('Admin.Categories.statusActive') : $t('Admin.Categories.statusInactive') }}
                  </span>
                </div>
                ...
                <div class="flex items-center gap-4 mt-2 text-sm text-gray-500">
                  <span v-if="category.product_count !== undefined">
                    {{ category.product_count }} {{$t('Admin.Categories.productsCount')}}
                  </span>
                  <span>•</span>
            ...
            <div class="flex items-center gap-2">
              ...
              <button @click="toggleActive(category)"
                ...
                {{ category.is_active ? $t('Admin.Categories.toggleDeactivate') : $t('Admin.Categories.toggleActivate') }}
              </button>
            </div>
          </div>
        </div>
      ...
      <div v-else class="flex flex-col items-center justify-center py-20">
        ...
        <p class="text-gray-600 text-lg font-medium mb-2">{{$t('Admin.Categories.emptyTitle')}}</p>
        <p class="text-gray-500 text-sm mb-4">{{$t('Admin.Categories.emptySubtitle')}}</p>
        <button @click="showAddModal = true"
      ...
      <div class="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md transform transition-all">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-2xl font-bold text-gray-900">
            {{ showEditModal ? $t('Admin.Categories.modalEditTitle') : $t('Admin.Categories.modalAddTitle') }}
          </h2>
        ...
        <form @submit.prevent="saveCategory" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">{{$t('Admin.Categories.nameLabel')}}</label>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">{{$t('Admin.Categories.descLabel')}}</label>
            <textarea v-model="formData.description" rows="3"
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-black/70"
              :placeholder="$t('Admin.Categories.descPlaceholder')"></textarea>
          </div>

          <div class="flex items-center gap-2">
            <input v-model="formData.is_active" type="checkbox" id="is_active"
              class="w-4 h-4 border-gray-300 rounded focus:ring-black" />
            <label for="is_active" class="text-sm font-medium text-gray-700">{{$t('Admin.Categories.activeLabel')}}</label>
          </div>
          ...
          <div class="flex justify-end gap-3 pt-4">
            ...
            <button type="submit"
              ...
              {{ showEditModal ? $t('Admin.Categories.update') : $t('Admin.Categories.create') }}
            </button>
          </div>
        </form>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md transform transition-all">
        ...
        <div class="flex justify-center gap-4">
          ...
          <button @click="confirm.onOk"
            ...
            {{$t('Admin.Categories.confirmTitle')}}
          </button>
        </div>
      </div>
    </div>
...
const saveCategory = async () => {
  try {
    ...
    toast.error(
      ...
      $t('Admin.Categories.actionFailed')
    )
  }
}

const confirmDelete = (id) => {
  confirm.message = $t('Admin.Categories.confirmDeleteMessage')
...
const toggleActive = async (category) => {
  try {
    ...
    toast.success(category.is_active ? $t('Admin.Categories.toggleSuccessDeactivate') : $t('Admin.Categories.toggleSuccessActivate'))
    await loadCategories()
  } catch (error) {
