...
class FavouriteProductRepository:
    ...
    async def get_by_user(self, user_id: str):
        ...
        if document:
            ...
            print(f"Found {product_count} favourite products for user {user_id}")
        else:
            print(f"No favourites found for user {user_id}")
        return document

    async def add_product(self, user_id: str, product_id: str):
        print(f"[FavouriteProductRepository] add_product: user_id={user_id}, product_id={product_id}")
        existing = await self.collection.find_one({"user_id": user_id})
        if existing:
            products = existing.get("product") or []
            if product_id in products:
                print(f"Product {product_id} already in favourites")
                return False
            ...
            print(f"Added product {product_id} to favourites. Total: {len(products) + 1}")
        else:
            await self.collection.insert_one({"user_id": user_id, "product": [product_id]})
            print(f"Created new favourite list for user {user_id}")
        return True
    ...
    async def remove_product(self, user_id: str, product_id: str):
        ...
        if result.modified_count > 0:
            print(f"Removed product {product_id} from user {user_id} favourites")
        else:
            print(f"Product {product_id} not found in user {user_id} favourites")
        return True
